...
class Dataset:
    ...
    def split(self, k_folds):
        ...
        for train_index, test_index in strat.split(x, y):
            ...
            y_train = self.data[train_index, -1]

            x_test = self.data[test_index, :-1]
            y_test = self.data[test_index, -1]
    ...
    def _get_ir(self):
        ...
        :return:
        """
        classe, count = np.unique(self.data[:, -1], return_counts=True)
...
if __name__ == "__main__":
    datasets = os.listdir("keel_ds/data/imbalanced/raw/")

    info = []

    for dataset in datasets:
        print(dataset)
        ds = Dataset(dataset[:-4], "data/imbalanced/raw/" + dataset)
